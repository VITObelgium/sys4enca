...
_min_version = "2.1.0-alpha.2"  # Minimum required package version for the plugin.
_version_next = "2.1.1"  # Next package version which may no longer be compatible with this version of the plugin.
_repo_url = (
    "https://artifactory.vgt.vito.be/artifactory/api/pypi/python-packages/simple"
)
...
def install_pip_deps():
    ...
    with tempfile.NamedTemporaryFile(
        mode="w+t", delete=False, prefix="sys4enca_constraints"
    ) as cf:
    ...
    try:
        ...
        QMessageBox.warning(
            None,
            "ENCA Installation",
            f"Installation of package {_package_dist_name} using pip failed.  "
...
def check_dependencies():
    ...
    try:
        ...
        QgsMessageLog.logMessage(
            (f"Current {_package_dist_name} version: {installed_version}."),
            level=Qgis.Info,
        )
        ...
        QgsMessageLog.logMessage(
            f"No version of {_package_dist_name} core package is currently installed.",
            level=Qgis.Info,
        )

    if run_pip:
        QgsMessageLog.logMessage(
            f"{_package_dist_name} version {_min_version} will be installed using pip.",
            level=Qgis.Info,
        )
        if not install_pip_deps():
            QMessageBox.warning(
                None,
                "ENCA Installation",
                "The installation is not complete.  The ENCA plugin may not "
                "work correctly.",
            )
            return False
    ...
    if parse_version(enca.__version__) < parse_version(_min_version):
        QMessageBox.warning(
            None,
            "Restart QGIS",
            f"The {_package_dist_name} package was updated.  Please restart "
