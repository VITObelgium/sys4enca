...
from qgis.gui import QgsFileWidget, QgsCheckableComboBox, QgsFilterLineEdit
...
def expand_template(template):
    ...
    if type(template) in (str, int, bool, float):
        return template
    if isinstance(template, QgsFileWidget):
        val = template.lineEdit().value().strip()  # Use lineEdit to correctly handle Default values such as 'Optional'
        if template.storageMode() == QgsFileWidget.GetMultipleFiles:  # List of files
            return template.splitFilePaths(val)
        return val
    if isinstance(template, QgsCheckableComboBox):
        user_data = template.checkedItemsData()
        if user_data is not None and len(user_data)>0:
            return [int(x) if isinstance(x,str) and x.isdigit() else x for x in user_data]
        else:
...
def writeWidget(widget, value):
    ...
    if isinstance(widget, QgsFileWidget):
        if not value:  # empty string, None, empty list...
            widget.lineEdit().setValue(None)
        elif isinstance(value, list):
            widget.setFilePaths(value)
        else:
...
def readWidget(widget):
    ...
    if isinstance(widget, QgsFileWidget):
        return widget.lineEdit().value().strip()
    elif isinstance(widget, QgsCheckableComboBox):
