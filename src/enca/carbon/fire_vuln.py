    ...
    def _start(self):
        for year in self.years:
            ...
            logger.debug('Calculate average fire severity rating for year %s.  Found %s netCDF input files.',
                         year, len(nc_files))
            with rasterio.open('netcdf:'+nc_files+':fdsrte') as src:
                profile = src.profile
                tags = src.tags()

                if (float(tags["fdsrte#GRIB_longitudeOfLastGridPointInDegrees"]) < 180.5):
                    affine = rasterio.transform.from_origin(float(tags["fdsrte#GRIB_longitudeOfFirstGridPointInDegrees"]),
                                                            float(tags["fdsrte#GRIB_latitudeOfFirstGridPointInDegrees"]),
                                                            float(tags["fdsrte#GRIB_iDirectionIncrementInDegrees"]),
                                                            float(tags["fdsrte#GRIB_jDirectionIncrementInDegrees"]))
                    data_roll = False
                else:
                    affine = rasterio.transform.from_origin(float(tags["fdsrte#GRIB_longitudeOfFirstGridPointInDegrees"])-180,
                                                            float(tags["fdsrte#GRIB_latitudeOfFirstGridPointInDegrees"]),
                                                            float(tags["fdsrte#GRIB_iDirectionIncrementInDegrees"]),
                                                            float(tags["fdsrte#GRIB_jDirectionIncrementInDegrees"]))
                    data_roll = True
            ...
            # ratio < 1 means lower vulnerability than LTA; > 1 means higher vulnerability / decrease in health
